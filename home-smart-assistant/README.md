...
giấc, lịch sự kiện, thời tiết cùng tin tức. Bộ não dùng một model có sẵn (qwen2.5 qua Ollama) kết hợp
...
1. **Model instruct, không có khối suy nghĩ.** Mặc định dùng `qwen2.5:7b-instruct` — model không
   sinh khối `<think>` dài trước khi trả lời, gọi công cụ (tool calling) ổn định hơn `qwen3:8b`.
   `ENABLE_THINKING`/`/no_think` chỉ còn tác dụng nếu bạn đổi `CHAT_MODEL` về một bản qwen3.
...
**Quan trọng — phần cứng quyết định phần còn lại.** Trên CPU thuần, `qwen2.5:7b-instruct` vẫn có thể
vượt 4 giây. Để chắc chắn đạt 2–4s:
- Chạy trên **GPU**, hoặc
- Dùng **model nhỏ hơn**: `CHAT_MODEL=qwen2.5:3b-instruct` (vẫn hỗ trợ tool calling), hoặc bản
  lượng tử hóa `q4`.
...
| `CHAT_MODEL` | `qwen2.5:7b-instruct` | Model chat (phải hỗ trợ tool calling) |
| `EMBED_MODEL` | `nomic-embed-text` | Model embedding |
| `ENABLE_THINKING` | `false` | Bật suy nghĩ (chỉ có tác dụng với model qwen3) |
